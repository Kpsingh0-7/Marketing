...
const createGupshupAppByName = async (token, name) => {
  try {
    ...
    if (err.response?.data?.message === "Bot Already Exists") {
      ...
      if (existingApp) return { appId: existingApp.id }; // <-- FIXED HERE
...
export const createGupshupApp = async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0 && rows[0].gupshup_id && rows[0].token) {
      ...
      // Step 2: Login
      const loginToken = await loginToGupshup(
        process.env.GS_EMAIL,
      ...
      const tokenResponse = await getAppToken(loginToken, appId);
      token = tokenResponse.token; // ✅ only use the string

      console.log("Insert values:", customer_id, gupshup_id, token);
    ...
    const onboarding = await generateOnboardingLink(
      token,
      gupshup_id,
      user,
      lang
