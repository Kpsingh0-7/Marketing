...
/* ---------------------------------------------------
   MAIN CONTROLLER (RAW WHATSAPP PAYLOAD SUPPORT)
----------------------------------------------------*/
export const sendWhatsappMessage = async (req, res) => {
  const {
    phoneNumber,
  ...
  try {
    ...
    /* ---------------------------------------------------
       RAW PAYLOAD HANDLING FOR ALL TYPES
    ----------------------------------------------------*/
    if (type === "text") {
      if (!req.body.text) {
        return res.status(400).json({
          success: false,
          error: "text object is required for type=text",
        });
      }
    ...
    if (type === "image") {
      if (!req.body.image) {
        return res.status(400).json({
          success: false,
          error: "image object is required for type=image",
        });
      }
    ...
    if (type === "video") {
      if (!req.body.video) {
        return res.status(400).json({
          success: false,
          error: "video object is required for type=video",
        });
      }
      payload.video = req.body.video;
    }
    ...
    if (type === "interactive") {
      if (!req.body.interactive) {
        return res.status(400).json({
          success: false,
          error: "interactive object is required for type=interactive",
        });
      }
    ...
    /* ---------------------------------------------------
       SAVE TO DB (SMART AUTO DETECTION)
    ----------------------------------------------------*/
    let contentToSave = "[message]";

    if (req.body.text?.body) contentToSave = req.body.text.body;
    if (req.body.image?.caption) contentToSave = req.body.image.caption;
    if (req.body.video?.caption) contentToSave = req.body.video.caption;
    if (req.body.interactive?.body?.text)
      contentToSave = req.body.interactive.body.text;
    ...
    const mediaToSave =
      ...
      req.body.video?.link ||
      req.body.video?.id ||
      req.body.interactive?.header?.image?.link ||
      req.body.interactive?.header?.image?.id ||
      null;
