...
export const addGroup = async (req, res) => {
  let connection;
  try {
    const { customer_id, group_name, description, contacts_json } = req.body;

    if (!customer_id || !group_name || !contacts_json) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields: customer_id, group_name, or contacts_json",
      });
    }

    const total_contacts = Array.isArray(contacts_json)
      ? contacts_json.length
      : 0;
    ...
    const [groupInsert] = await connection.execute(
      `INSERT INTO contact_group 
        (group_name, description, customer_id, contacts_json, total_contacts)
       VALUES (?, ?, ?, ?, ?)`,
      [group_name.trim(), description?.trim() || null, customer_id, JSON.stringify(contacts_json), total_contacts]
    );
    ...
    return res.status(201).json({
      success: true,
      message: `Group '${group_name}' created with ${total_contacts} contact(s).`,
      data: {
        group_id: groupInsert.insertId,
        group_name,
        total_contacts,
